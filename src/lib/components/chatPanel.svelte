<script lang="ts">
    ...
    function getUpdatedConversation(conversations: Conversation[], curConversationId: string) {
        return conversations.find((c) => c.id === curConversationId)
    }

    function addNewMessageToConversation(newMessage: Message) {
        conversations.update((conversations) => {
            const conversation = conversations.find((c) => c.id === $curConversationId)
            if (conversation) {
...
<div class="w-full pb-4 pt-4 flex flex-col ml-72 mr-72 mt-20 mb-20 overflow-x-auto">
    {#if $curConversation}
        <div class="chat-container flex-grow">
            {#each $curConversation.messages as message}
                <div class="chat-header">{message.role}</div>
                <div class="p-2 m-2 rounded {message.role === 'user' ? 'user-message' : 'assistant-message'}">
                    <SvelteMarkdown
    ...
    <div class="fixed right-72 left-72 bottom-0 flex items-center p-4 bg-base-100">
        <div class="mb-2 flex w-full items-center">
            ...
            <button on:click={sendMessage} class="btn ml-2 p-2" disabled={isStreaming}> <ArrowRightSolid /></button>
        </div>
    </div>
</div>
