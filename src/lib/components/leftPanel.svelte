<script lang="ts">
    ...
    function getLocalObjects() {
        if (localStorage) {
            const storedObjects = JSON.parse(localStorage.getItem(objectName));
    ...
    function exportData() {
        const objectJson = JSON.stringify($objects, null, 2);
...
<div class="w-72 p-4 pb-60 h-full fixed top-20 left-0">
    <button class="btn mb-2 w-full justify-between" on:click={newObject}>
        New {objectName}
        <PlusOutline />
    </button>
    <input
