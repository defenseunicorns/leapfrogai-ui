...
<input class="input input-bordered w-full mb-2 mt-2" type="text" placeholder="Search" bind:value={documentSearch} />
{#if $documents.length > 0}
    <div class="menu">
        {#each $documents as document}
            {#if documentSearch === '' || document.source.toLowerCase().includes(documentSearch.toLowerCase())}
                <div class="my-1 flex ml-1 flex-row w-full content-center">
                    ...
                    <button
                        ...
                        on:mouseover={() => {
                            currentDocumentId.set(document.uuid)
                        }}
                        on:focus={onfocus}
                        on:click={() => removeDocument($currentDocumentId)}
                    >
                        <TrashBinOutline />
                    </button>
                </div>
