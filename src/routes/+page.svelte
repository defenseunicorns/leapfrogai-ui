...
async function sendMessage() {
    ...
    let conversationMessages = [...$conversations.find((c) => c.id === $currentConversation).messages]
    ...
    if (ragEndpointActive && ragEnabled) {
        ...
        conversationMessages.splice(conversationMessages.length - 1, 0, ragResponse)
    }
...
async function regenerateResponse() {
    ...
    $conversations[$currentConversation].messages = $conversations[$currentConversation].messages.filter(
        (message, index, self) => {
...
<div class="flex flex-col h-screen bg-dark-blue {$theme}">
    ...
    <div class="flex flex-grow">
        ...
        <div class="w-1/5 bg-blue-800 p-4 flex flex-col text-white">
                            ...
                            <div class="flex">
                                {#if editingConversationIndex === index}
                                    <input
                                        ...
                                        autofocus />
                                {:else}
                                    <button class="btn" on:click="{() => currentConversation.set(conversation.id)}"
                                        >{conversation.name}</button>
                                    <button class="btn" on:click="{() => startEditingConversationName(index)}"
                                        ><EditOutline /></button>
                                {/if}
                                <button class="btn" on:click="{() => removeConversation(index)}"
            ...
            {/if}
            <button class="btn mb-2" on:click="{clearConversations}">Clear conversations</button>
                    ...
                    {#each $folders as folder, index}
                        <div class="flex">
                            ...
                            {:else}
                                ...
                                <button class="btn" on:click="{() => startEditingFolderName(index)}"
                                    ><EditOutline /></button>
                            {/if}
                            <button class="btn" on:click="{() => removeFolder(index)}"><TrashBinSolid /></button>
                        </div>
                    {/each}
                </div>
            {/if}
            <button class="btn mb-2" on:click="{clearFolders}">Clear Folders</button>
            ...
            {#if ragEndpointActive}
                <input
                    ...
                    multiple="multiple"
                    class="hidden" />
                <button class="btn mb-2" on:click="{() => fileInputRag.click()}">Import files</button>
            {/if}
            <input type="file" accept=".json" on:change="{importData}" bind:this="{fileInput}" class="hidden" />
            <button class="btn mb-2" on:click="{() => fileInput.click()}">Import data</button>
            <button class="btn mb-2" on:click="{exportData}">Export data</button>
            <button class="btn mb-2" on:click="{() => (showSettingsModal = !showSettingsModal)}">Settings</button>
        </div>

        <!-- Center Panel -->
        <div class="w-3/5 bg-blue-600 p-4 flex flex-col text-white">
            <div bind:this="{chatContainer}" class="chat-container mb-2 overflow-auto flex-grow">
            ...
            <button class="btn mb-2" on:click="{regenerateResponse}">Regenerate Response</button>
            <div class="mb-2 flex items-center">
                <form on:submit|preventDefault="{sendMessage}" class="mb-2 flex-grow items-center">
                    <Input
                        id="small-input"
                        size="sm"
                ...
                <button class="btn ml-2 p-2" on:click="{() => (showSettingsModal = !showSettingsModal)}">
                    <UserSettingsSolid />
                </button>
            </div>
        </div>
        ...
        <div class="w-1/5 bg-blue-800 p-4 flex flex-col text-white">
            ...
            {#each $personas as persona}
                {#if personaSearch == '' || persona.name.toLowerCase().includes(personaSearch.toLowerCase())}
                    <div class="menu bg-base-200 w-full rounded-box">
                        <div class="flex">
                            <button
                                class="btn"
                            ...
                            <button class="btn" on:click="{() => removePersona(persona.id)}"><TrashBinSolid /></button>
                        </div>
                    </div>
                {/if}
            {/each}
            {#if showPersonaDetails}
                <Heading class="underline-heading" tag="h5">Persona Details</Heading>
                <div class="mb-2">
                    <input
                        id="persona-name"
                        type="text"
...
{#if showSettingsModal}
    <div class="fixed inset-0 flex items-center justify-center z-10">
        <div class="modal-box p-4 rounded shadow-lg">
            ...
            <div class="mb-2">
                <label for="temperature">Temperature:</label>
                <input id="temperature" type="range" min="0" max="1" step="0.01" bind:value="{temperature}" />
            </div>
            {#if ragEndpointActive}
                <div class="mb-2">
                    <div>
                        <span>Retrieval Augmented Generation</span>
                        <input type="checkbox" class="checkbox" bind:checked="{ragEnabled}" />
                    </div>
                </div>
            {/if}
            ...
            <form on:submit|preventDefault="{updatePersona}">
                ...
                <div class="modal-action">
                    ...
                    <button
                        type="button"
                        class="btn"
                        on:click="{() => document.getElementById('persona_modal').close()}">Close</button>
                </div>
            </form>
        </div>
    </dialog>
