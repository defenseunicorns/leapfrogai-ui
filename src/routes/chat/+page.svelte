<script lang="ts">
	...
	function newChat(): string {
		chatUuid = uuidv4();
		conversations.update((n) => [...n, { id: chatUuid, name: 'New conversation', messages: [] }]);
	...
	<div class="flex flex-grow">
										...
										{:else}
											<li class="w-4/6 flex-nowrap">
												<button
													...
													on:click={async () => currentConversation.set(conversation.id)}
												>
		...
		<div class="w-full pb-4 pt-4 flex flex-col ml-72 mr-72 mt-20 mb-20 overflow-x-scroll">
			{#if $currentConversation !== null && $conversations.find((c) => c.id === $currentConversation)}
				<div bind:this={chatContainer} class="chat-container overflow-auto flex-grow">
