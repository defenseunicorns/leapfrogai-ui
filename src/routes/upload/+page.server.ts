...
const createCompletion = async (openaiClient: OpenAI, model: string, prompt: string, maxTokens: number) => {
...
export const actions = {
    upload: async ({ request }: RequestEvent) => {
        ...
        console.log(`Started new workflow for ${filename} (${audioFile.type}) of size ${audioFile.size / 1000000}MB.`)
    ...
    summarize: async ({ request }: RequestEvent) => {
        ...
        const tokenizedTranscript = tokenize(transcription)
